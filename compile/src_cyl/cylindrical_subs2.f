...
!              => u = (M^-1)v
      ...
      integer ntot
      real glsc2,glsc3,vlsc2,vlsc3,glsc2_full_M1
      ...
      logical ifprec
      logical iffull          ! scalar products with dense mass matrix
      ...
      if (ifwgt) then
        if (iffull) then
          call rone(wp,ntot)
          alpha = glsc2_full_M1(wp,wp)
          norm_fac = 1.0/sqrt(alpha)
        else  
...
!              => u = (M^-1)v
      ...
      integer nt1,nt3,lt3
      real glsc2,glsc3,vlsc2,vlsc3,op_glsc2_wt,glsum
      integer iconv,intype
      real tolpss,div0
      integer i,k,iter
      real etime2,etime_p,ratio,rnorm
